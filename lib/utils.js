...
const STORE_DATA_PATH = path.join(process.cwd(), 'data/shop');
...
export function getStoreCollectionsWithProducts() {
    const source = fs.readFileSync(path.join(STORE_DATA_PATH, 'collections.json'));
    const collections = JSON.parse(source);

    Object.keys(collections).forEach(collectionSlug => {
        collections[collectionSlug] = getCollectionBySlug({ slug: collectionSlug })
    })
