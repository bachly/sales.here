...
import { postFilePaths, POSTS_PATH, samplePostFilePaths, SAMPLE_POSTS_PATH } from './mdxUtils';
...
export function getAllSamplePosts() {
    const allPosts = samplePostFilePaths.map((filePath) => {
        const fullPath = path.join(SAMPLE_POSTS_PATH, filePath);
        const source = fs.readFileSync(fullPath)
        const stat = fs.statSync(fullPath)
        const { data } = matter(source) // { content, data }
        const slug = filePath.replace(/\.mdx$/, '')
